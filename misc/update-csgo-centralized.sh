#!/bin/bash
# KitsuneLab CSGO Centralized Update Script
# Automatically updates CSGO files and pushes them to all server containers
#
# Usage: ./update-csgo-centralized.sh [--simulate] [--validate]
#        ./update-csgo-centralized.sh --daemon
#
...
#                 a CSGO container starts (new server or restart). Install as a
...
# "symlink"  = symlinks to CSGO_DIR, panel sees ~0 disk usage per server (default)
#              CSGO_DIR is bind-mounted read-only into each container automatically
#              requires kernel 5.2+ (Ubuntu 20.04+), python3 on the host
# "hardlink" = hardlinks to CSGO_DIR inodes, zero extra REAL disk space but panel
#              disk quota counts full size (~53GB) - use only if quota doesn't matter
#              falls back to copy if CSGO_DIR and panel volumes are on different filesystems
...
# Optional: When to push the non-VPK base files (bin/, platform/, srcds_linux, csgo/*)
# into a server volume. VPK symlinks are always refreshed - this only controls the
# rsync of everything else.
#
# "auto"   = first boot, or when the shared install's buildid changes (default)
# "always" = every push. WARNING: this overwrites anything in csgo/ that also exists
#            in the shared install, on every container start. Files under csgo/addons/
#            and csgo/cfg/ are safe (excluded), everything else silently reverts.
# "never"  = never push base files. Volumes must already have them.
BASE_FILES_SYNC="auto"
...
validate_config() {
    ...
    # Validate CSGO_DIR path (must be absolute, no special chars except /-_)
    if [[ ! "$CSGO_DIR" =~ ^/[a-zA-Z0-9/_-]+$ ]]; then
        log_error "Invalid CSGO_DIR path: $CSGO_DIR"
    ...
    # Validate BASE_FILES_SYNC value
    case "$BASE_FILES_SYNC" in
        auto|always|never) ;;
        *)
            log_error "Invalid BASE_FILES_SYNC: $BASE_FILES_SYNC (must be: auto, always, never)"
            ((errors++))
            ;;
    esac
...
acquire_lock() {
    ...
    exec 200>"$lockfile"
    if ! flock -n 200; then
        log_error "Another CSGO update instance is already running"
        ...
        if grep -q "state is 0x" "$log_file" 2>/dev/null; then
            ...
            case "$error_code" in
                0x202)
                    ...
                    log_info "• CSGO requires ~60GB for initial installation"
                    ...
                    log_info "Check space: ${BOLD}df -h $(dirname "$CSGO_DIR")${RESET}"
                    ;;
                *)
...
get_local_version() {
    ...
    local manifest="$CSGO_DIR/steamapps/appmanifest_$APP_ID.acf"
    if [ -f "$manifest" ]; then
...
update_cs2() {
    ...
    if ! run_with_live_tail "Checking for updates and downloading" \
        "$STEAMCMD_DIR/steamcmd.sh" +force_install_dir "$CSGO_DIR" +login anonymous +app_update "$APP_ID" $validate_flag +quit; then
        log_error "CSGO update failed"
        exit 1
    fi
    ...
    if [ "$version_before" = "$version_after" ]; then
        log_ok "CSGO is already up to date (version: $version_after)"
    else
        if [ "$version_before" = "unknown" ]; then
            log_ok "CSGO installed successfully (version: ${BOLD}$version_after${RESET})"
        else
            log_ok "CSGO updated successfully: $version_before → ${BOLD}$version_after${RESET}"
        fi
    fi

    # Install Steam client libraries
    mkdir -p "$CSGO_DIR/.steam/sdk32" "$CSGO_DIR/.steam/sdk64"
    cp -f "$STEAMCMD_DIR/linux32/steamclient.so" "$CSGO_DIR/.steam/sdk32/" 2>/dev/null || true
    cp -f "$STEAMCMD_DIR/linux64/steamclient.so" "$CSGO_DIR/.steam/sdk64/" 2>/dev/null || true
...
# Bind-mount CSGO_DIR into a running container's mount namespace via nsenter.
...
# Sync base files + VPK files from CSGO_DIR into a single server volume
# Decide whether the non-VPK base files should be rsynced into this volume.
# Returns 0 = sync, 1 = skip.
#
# The old behaviour was to rsync on every push. That silently reverted anything
# under csgo/ that also exists in the shared install, on every container start.
# Now we sync on first boot, or when the shared install's buildid actually changes.
_should_sync_base_files() {
    local dest="$1"
    local stamp="$dest/egg/.base-files-buildid"
    ...
    # First boot: no engine binary yet.
    [ ! -f "$dest/srcds_linux" ] && return 0

    local current
    current=$(get_local_version)
    # Unknown buildid means we can't tell. Don't touch a populated volume.
    [ "$current" = "unknown" ] && return 1

    # Volume predates the stamp: adopt the current buildid and leave files alone.
    if [ ! -f "$stamp" ]; then
        echo "$current" > "$stamp" 2>/dev/null || true
        chown pterodactyl:pterodactyl "$stamp" 2>/dev/null || true
        return 1
    fi

    [ "$(cat "$stamp" 2>/dev/null)" != "$current" ]
}
...
_sync_to_volume() {
    ...
    local container_mount_dst="/tmp/csgo-shared"
    ...
    # Sync non-VPK base files; exclude per-server configs, gameinfo.txt, and
    # SteamCMD-only dirs (Steam/, steamapps/) — the CSGO server doesn't need them at
    ...
    if _should_sync_base_files "$dest"; then
        rsync -aK --no-o --no-g \
            --exclude '*.vpk' \
            --exclude 'cfg/' \
            --exclude 'csgo/addons/' \
            --exclude 'csgo/gameinfo.txt' \
            --exclude 'csgo/mapcycle.txt' \
            --exclude 'csgo/maplist.txt' \
            --exclude 'Steam/' \
            --exclude 'steamapps/' \
            "$src/" "$dest" 2>/dev/null || {
        ...
        get_local_version > "$dest/egg/.base-files-buildid" 2>/dev/null || true
        chown pterodactyl:pterodactyl "$dest/egg/.base-files-buildid" 2>/dev/null || true
        log_info "  ${DIM}→ $container: base files synced${RESET}"
    fi

    # Ensure volume root stays writable for the container user
    chmod 755 "$dest" 2>/dev/null || true

    # Copy gameinfo.txt only on first sync - don't overwrite the server's own
    local gameinfo_src="$src/csgo/gameinfo.txt"
    local gameinfo_dst="$dest/csgo/gameinfo.txt"
    if [ -f "$gameinfo_src" ] && [ ! -f "$gameinfo_dst" ]; then
        cp "$gameinfo_src" "$gameinfo_dst" 2>/dev/null || true
    fi

    # Copy map lists only on first sync - a server's own map cycle must survive reboots
    local map_file
    for map_file in mapcycle.txt maplist.txt; do
        if [ -f "$src/csgo/$map_file" ] && [ ! -f "$dest/csgo/$map_file" ]; then
            cp "$src/csgo/$map_file" "$dest/csgo/$map_file" 2>/dev/null || true
        fi
    done

    # Copy cfg files - only if they don't already exist (never overwrite)
    local cfg_src="$src/csgo/cfg"
    local cfg_dst="$dest/csgo/cfg"
    if [ -d "$cfg_src" ]; then
    ...
    chown -R pterodactyl:pterodactyl "$dest/csgo" 2>/dev/null || true
...
# Verify every VPK present in CSGO_DIR is properly placed on a server volume.
...
_verify_volume_vpks() {
    ...
    done < <(find "$CSGO_DIR" -type f -name "*.vpk" -print0 2>/dev/null)
...
push_vpk_to_containers() {
    ...
    # symlink mode: ensure CSGO_DIR is world-readable so container user can access the mount
    if [ "$VPK_PUSH_METHOD" = "symlink" ]; then
        chmod -R a+rX "$CSGO_DIR" 2>/dev/null || true
    fi
    ...
    # Hardlink mode: set VPK files in CSGO_DIR to root:root 644
...
run_event_daemon() {
    ...
    log_info "CSGO source: ${BOLD}$CSGO_DIR${RESET}"
    echo "" >&2
    ...
    while true; do
        ...
        while IFS=' ' read -r event container; do
            ...
            if [ "$event" = "start" ] && [ "$VPK_PUSH_METHOD" = "symlink" ]; then
                if _nsenter_mount "$container" "$CSGO_DIR" "/tmp/csgo-shared"; then
                    log_info "CSGO_DIR mounted into ${BOLD}$container${RESET} at /tmp/csgo-shared"
                    local _vol_path
            ...
            local debounce_file="/tmp/csgo-vpk-pushed-${container}"
            ...
            local lock_file="/var/lock/csgo-vpk-push-${container}.lock"
...
apply_update() {
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet csgo-vpk-daemon 2>/dev/null; then
        log_info "Restarting csgo-vpk-daemon service to load new code..."
        systemctl restart csgo-vpk-daemon 2>/dev/null || log_warn "Daemon restart failed - run: systemctl restart csgo-vpk-daemon"
    fi
...
main() {
    ...
    headline "KitsuneLab CSGO Centralized Update"
    ...
    log_info "CSGO Directory: ${BOLD}$CSGO_DIR${RESET}"
    ...
    if [ "$SIMULATE_MODE" = "true" ]; then
        ...
        log_ok "CSGO update completed successfully"
    fi

    log_info "Version:    ${BOLD}$(get_local_version)${RESET}"
    log_info "Location:   ${BOLD}$CSGO_DIR${RESET}"
    log_info "Push method: ${BOLD}$VPK_PUSH_METHOD${RESET}"
